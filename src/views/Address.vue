<template>
    <div class="checkout-page">
        ...
        <div class="container">
            <div class="checkout-addr">
                    ...
                    <div class="addr-list">
                        <ul>
                            <li v-for="(item, index) in addressListFilter"
                                :class="{'check': checkIndex===index}" @click="checkIndex=index; selectedAddrId=item.addressId">
                                <dl>
                                    <dt>{{item.userName}}</dt>
                            ...
                            <li class="addr-new" @click="addAddrFlag=true">
                ...
                <div class="next-btn-wrap">
                    <button class="btn btn--m btn--red" @click="toOrderConfirm">
                        Next
                    </button>
                </div>
            </div>
        </div>
        <v-modal :mdShow="isMdShow" @closeModal="isMdShow=false">
        ...
        <v-modal :mdShow="addAddrFlag" @closeModal="addAddrFlag=false">
            <div class="md-title" slot="title">Add new address</div>
            <div class="confirm-tips" slot="message">
                <div class="error-wrap">
                    <span class="error error-show" v-show="errorTip">{{errorMsg}}</span>
                </div>
                <ul>
                    <li class="regi_form_input">
                        <input v-model="newAddress.addressId" type="text" tabindex="1" name="addressId" class="regi_login_input regi_login_input_left" placeholder="Address Id" data-type="loginname">
                    </li>
                    <li class="regi_form_input noMargin">
                        <input v-model="newAddress.userName" type="text" tabindex="2"  name="userName" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="userName" >
                    </li>
                    <li class="regi_form_input noMargin">
                        <input v-model="newAddress.streetName" type="text" tabindex="2"  name="streetName" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="streetName" >
                    </li>
                    <li class="regi_form_input noMargin">
                        <input v-model="newAddress.tel" type="text" tabindex="2"  name="telephone" class="regi_login_input regi_login_input_left login-input-no input_text" placeholder="telephone" >
                    </li>
                </ul>
            </div>
            <div slot="btnGroup">
                <a href="javascript:;" class="btn btn--m" @click="addNewAddress">添  加</a>
                <a href="javascript:;" class="btn btn--m" @click="addAddrFlag=false">取消</a>
            </div>
        </v-modal>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        this.$ajax.get('/users/addressList').then((res) => {
          let data = res.data;
      ...
      setDefault(addressId) {
        this.$ajax.post('/users/setDefault', {
          addressId: addressId
        }).then((res) => {
          let data = res.data;
          if(data.status === '0') {
      ...
      delAddress() {
        this.$ajax.post('/users/delAddress',{
      ...
      addNewAddress() {
        if(this.newAddress.addressId==='' || this.newAddress.userName==='' ||
          this.newAddress.streetName==='' || this.newAddress.tel===''){
          this.errorTip = true;
          this.errorMsg = '以下填空不能为空';
        } else if(!parseInt(this.newAddress.tel)){
          this.errorTip = true;
          this.errorMsg = '电话必须为数字';
          console.log(typeof (this.newAddress.tel))
        }else{
          this.$ajax.post('/users/addNewAddress', {
